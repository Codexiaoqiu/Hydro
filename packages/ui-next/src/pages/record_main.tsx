...
export default function RecordMainPage() {
  ...
  useEffect(() => {
    ...
    if (!rids.length || typeof EventSource === 'undefined') return undefined;
    const url = UiContext?.socketUrl
      ?? `/record-conn?${rids.map((id) => `rid=${encodeURIComponent(id)}`).join('&')}`;
    const es = new EventSource(url);
    es.addEventListener('update', (ev) => {
      try {
        const data = JSON.parse((ev as MessageEvent).data) as {
          rid: string; status?: number; score?: number;
        };
  ...
  return (
    <main style={{ maxWidth: 1320, margin: '0 auto', padding: 'var(--space-6)' }}>
      <header style={{ marginBottom: 'var(--space-4)' }}>
        <h1 style={{
          fontFamily: 'var(--font-display)', fontSize: 'var(--text-2xl)', margin: 0,
        }}
        >
          {t('RecordMain.Title')}
        </h1>
        {pdoc?.title && (
          <p style={{ color: 'var(--text-mute)', margin: 'var(--space-1) 0 0' }}>
            {pdoc.title}
          </p>
        )}
      </header>
      ...
      >
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <input
            value={query.uidOrName}
            onChange={(e) => setQuery({ ...query, uidOrName: e.currentTarget.value })}
            style={inputStyle}
            placeholder={t('RecordMain.UserPlaceholder')}
          />
        </label>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <input
            value={query.pid}
            onChange={(e) => setQuery({ ...query, pid: e.currentTarget.value })}
            style={inputStyle}
            placeholder={t('RecordMain.ProblemPlaceholder')}
          />
        </label>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <select
            ...
            onChange={(e) => setQuery({ ...query, language: e.currentTarget.value })}
            style={inputStyle}
          >
        ...
        <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <select
            value={query.status}
            onChange={(e) => setQuery({ ...query, status: e.currentTarget.value })}
            style={inputStyle}
          >
            <option value="">{t('Common.Any')}</option>
            {STATUS_KEYS.map((s, i) => (
              <option key={i} value={String(i)}>{t(s)}</option>
            ))}
          </select>
        </label>
        <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
          <label>
            <input
              type="checkbox"
              checked={query.all}
              onChange={(e) => setQuery({ ...query, all: e.currentTarget.checked })}
            />
            {' '}
            {t('Common.All')}
          </label>
          <label>
            <input
              type="checkbox"
              checked={query.allDomain}
              onChange={(e) => setQuery({ ...query, allDomain: e.currentTarget.checked })}
            />
            {' '}
            {t('RecordMain.AllDomains')}
          </label>
        </div>
        ...
        <tbody>
          {merged.map((r) => (
            <tr key={r._id} style={{ borderTop: '1px solid var(--border)' }}>
              <td style={td}>
                <Link to="record_detail" params={{ rid: String(r._id) }}>
              ...
              <td style={td}>
                {r.pid
                  ? <Link to="problem_detail" params={{ pid: String(r.pid) }}>{String(r.pid)}</Link>
                  : '—'}
              </td>
              <td style={td}>{r.lang ?? '—'}</td>
              <td style={td}>
                {r.status !== undefined && STATUS_KEYS[r.status]
                  ? t(STATUS_KEYS[r.status])
                  : t('Record.Status.Pending')}
              </td>
              <td style={td}>{r.score ?? '—'}</td>
              <td style={td}>
                {r.judgeAt ? new Date(r.judgeAt).toLocaleTimeString() : '—'}
              </td>
            </tr>
          ))}
          {!merged.length && (
            <tr>
              <td style={td} colSpan={7}>
                <em style={{ color: 'var(--text-mute)' }}>
                  {t('RecordMain.NoSubmissions')}
                </em>
              </td>
            </tr>
          )}
